...
export default function AssignmentEditor() {
    ...
    const loadAssignment = async () => {
        if (aid && aid !== "new") {
            try {
                const a = await assignmentClient.findAssignmentById(aid);
                setTitle(a.title || "");
                setDescription(a.description || "");
                setPoints(parseInt(a.points?.replace(/\D/g, "") || "0"));
                setDueDate(
                    a.due_date
                        ? format(parse(a.due_date, "MMM dd yyyy hh:mma", new Date()), "yyyy-MM-dd'T'HH:mm")
                        : ""
                );
                setAvailability(
                    a.availability
                        ? format(parse(a.availability, "MMM dd yyyy hh:mma", new Date()), "yyyy-MM-dd'T'HH:mm")
                        : ""
                );
            } catch (err) {
    ...
    return (
        <div id="wd-assignments-editor">
            ...
            <br />
            {/* Rest of form unchanged — keep your group, grade, submission sections here */}
            {/* Form for Assignment Details */}
            <form className="d-flex flex-column">
                {/* Points Input */}
                <div className="d-flex align-content-center ms-auto">
            ...
            <hr/>
            <div id="wd-modules-controls" className="text-nowrap">
                <Button variant="danger" size="lg" className="me-1 float-end" style={{ width: "150px" }} onClick={handleSave}>
                    Save
                </Button>
                <Button variant="secondary" size="lg" className="me-1 float-end" style={{ width: "150px" }}
